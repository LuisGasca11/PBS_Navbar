...
const DropdownMenu = ({ title, badge, items, icon, isWide = false, isMobileMenuOpen, setIsMobileMenuOpen }) => {
  ...
  return (
    <div 
      ...
      onMouseEnter={() => !isMobileMenuOpen && setIsOpen(true)}
      onMouseLeave={() => !isMobileMenuOpen && setIsOpen(false)}
    >
      <button 
      ...
      {isOpen && (
        <div className={`absolute left-0 top-full mt-2 ${isWide ? 'w-80' : 'w-64'} 
          bg-[#1a1a1a]/95 backdrop-blur-md rounded-lg shadow-2xl border border-gray-700/50 z-50 ${
            isMobileMenuOpen ? 'relative top-0 w-full mt-2' : ''
          }`}>
          <div className="py-4">
            {items.map((item, idx) => (
              <div key={idx}>
